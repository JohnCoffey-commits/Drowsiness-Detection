#!/usr/bin/env python3
"""Build the English final PDF report for the drowsiness project."""
...
import build_final_report as zh_builder


ROOT = Path(__file__).resolve().parents[2]
OUT_DIR = ROOT / "docs" / "final"
FIG_DIR = OUT_DIR / "figures"
PDF_PATH = OUT_DIR / "final_report_en.pdf"
MD_PATH = OUT_DIR / "final_report_en.md"
PALETTE = zh_builder.PALETTE
...
SECTIONS = [
    (
        ...
        "fig01_system_pipeline.png",
        "Figure 1. Project-level pipeline: data preparation, specialist models, temporal rules, fusion, and application-facing outputs.",
    ),
    (
    ...
    (
        ...
        "fig02_data_processing_flow.png",
        "Figure 2. Data processing flow: YawDD/YawDD+ Dash mouth evidence and MRL Eye evidence are prepared independently with leakage-aware subject-level splits.",
    ),
    (
    ...
    (
        ...
        "fig03_fusion_logic.png",
        "Figure 3. F5 fusion logic: signal quality is checked before mouth-eye evidence is elevated into warning-candidate states.",
    ),
    (
        ...
        "fig04_model_performance.png",
        "Figure 4. Specialist model performance. These are module-level results, not final system-level drowsiness accuracy.",
    ),
    (
        ...
        "fig05_stage15_fusion_counts.png",
        "Figure 5. Stage 15 fusion state counts for A/B/C/D controlled validation videos.",
    ),
    (
...
def metric_table(rows: list[list[str]], widths: list[float] | None = None) -> Table:
...
def add_figure(story: list, filename: str, caption: str, st: dict[str, ParagraphStyle], width_mm: float = 160) -> None:
    ...
    img.hAlign = "CENTER"
    story.extend([Spacer(1, 4), img, para(caption, st["caption"]), Spacer(1, 5)])
...
def write_markdown() -> None:
    ...
    for heading, paragraphs, fig, caption in SECTIONS:
        lines.extend([f"## {heading}", ""])
        for paragraph in paragraphs:
            lines.extend([paragraph, ""])
        if fig:
            lines.extend([f"![{caption}](figures/{fig})", ""])
        if heading == "5. Results and Evaluation":
            lines.extend(markdown_table(MOUTH_RESULTS))
            lines.extend(markdown_table(EYE_RESULTS))
        if heading == "5.1 Controlled Fusion Validation":
            lines.extend(markdown_table(FUSION_RESULTS))
...
def build_pdf() -> None:
    ...
    doc = ReportDocTemplate(
        ...
        topMargin=20 * mm,
        bottomMargin=18 * mm,
        title="Evidence-Fused Driver Drowsiness Warning-Candidate System",
        author="Drowsiness Detection Project Group",
    )
    ...
    story: list = [
        Spacer(1, 56 * mm),
        Paragraph("Evidence-Fused Driver Drowsiness<br/>Warning-Candidate System", st["title"]),
        Paragraph("Final Technical Report | Drowsiness Detection Project", st["subtitle"]),
        metric_table(
            [
                ["Item", "Value"],
                ...
                ["Generated at", "docs/final/final_report_en.pdf"],
            ],
            [38 * mm, 132 * mm],
        ),
        Spacer(1, 28 * mm),
        para(
            "Claim boundary: this report documents specialist-module metrics and controlled-validation warning-candidate behavior. It does not claim final system-level drowsiness accuracy, deployment readiness, clinical validation, or real-world road validation.",
            st["small"],
        ),
        PageBreak(),
        Paragraph("Table of Contents", st["toc_h"]),
        toc,
        PageBreak(),
    ]

    for heading, paragraphs, fig, caption in SECTIONS:
        style = st["h2"] if heading.startswith("5.1") else st["h1"]
        story.append(Paragraph(heading, style))
        for paragraph in paragraphs:
            story.append(para(paragraph, st["body"]))
        if fig and caption:
            add_figure(story, fig, caption, st)
        if heading == "5. Results and Evaluation":
            story.append(Paragraph("Table 1. Stage 7 mouth/yawn specialist results", st["h2"]))
            story.append(metric_table(MOUTH_RESULTS, [28 * mm, 22 * mm, 22 * mm, 22 * mm, 28 * mm, 26 * mm, 22 * mm]))
            story.append(Spacer(1, 5))
            story.append(Paragraph("Table 2. Stage 9/9B MRL Eye specialist results", st["h2"]))
            story.append(metric_table(EYE_RESULTS, [30 * mm, 22 * mm, 22 * mm, 25 * mm, 23 * mm, 25 * mm, 24 * mm]))
            story.append(Spacer(1, 6))
        if heading == "5.1 Controlled Fusion Validation":
            story.append(Paragraph("Table 3. Stage 15 F5 fusion counts", st["h2"]))
            story.append(metric_table(FUSION_RESULTS, [56 * mm, 19 * mm, 19 * mm, 22 * mm, 34 * mm, 30 * mm]))
            story.append(Spacer(1, 6))
